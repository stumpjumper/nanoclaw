...
function syncSkillSymlinks(claudeDir: string, containerConfig: import('./container-config.js').ContainerConfig): void {
  ...
  for (const skill of desired) {
    ...
    if (!entry) {
      ...
      // so a stale one is dangling *inside* the container while looking fine on
      // the host — the skill simply isn't there, and the agent reports the
      // script as missing. Reconciling only presence (create missing, drop
      // unwanted) left these untouched and silently broken.
