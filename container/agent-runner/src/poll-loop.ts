...
export async function processQuery(
  ...
  // — see getToolSentSince in db/messages-out.ts. Advanced past this turn's
  // own writes after each result event so one turn's sends never dedup a
  // later turn's.
  ...
  try {
    for await (const event of query.events) {
      ...
      if (event.type === 'init') {
        ...
        if (event.text) {
          const { sent, hasUnwrapped, taskBlocks } = dispatchResultText(event.text, routing, toolSentTexts);
...
export function dispatchResultText(
  ...
  // `!routing.taskRun`: under upstream's one-door task delivery a task run's
  // plain final text is already auto-appended to the run log, so delivering it
  // to chat here would double-deliver it.
  if (sent === 0 && scratchpad && !scratchpadIsDuplicate && !routing.taskRun) {
  ...
  // Duplicate scratchpad (already sent via a message tool this turn) is not
  // undelivered either — nudging would re-send content the user already has.
  const hasUnwrapped = !routing.taskRun && sent === 0 && !!scratchpad && !scratchpadIsDuplicate;
  if (hasUnwrapped) {
